...
class CASTModel(BaseModel):
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN_G_A > 0.0:
            ...
            self.real_A_features = self.clip_model.encode_image(self.clip_normalize(self.real_A, self.device))
            self.real_A_features /= (self.real_A_features.clone().norm(dim=-1, keepdim=True))

            self.rec_netD_B=self.netD_B(self.real_A)
            self.rec_netD_B_features = self.clip_model.encode_image(self.clip_normalize(self.rec_netD_B, self.device))
            self.rec_netD_B_features /= (self.rec_netD_B_features.clone().norm(dim=-1, keepdim=True))
            self.loss_detail_loss = (1 - torch.cosine_similarity(self.real_A_features, self.rec_netD_B_features, dim=1)).mean()*0.05
        else:
            self.loss_G_A = 0.0
        ...
        self.loss_G = self.loss_cyc + self.loss_G_NCE_style + (self.loss_G_A + self.loss_G_B) * 0.5+self.loss_detail_loss
